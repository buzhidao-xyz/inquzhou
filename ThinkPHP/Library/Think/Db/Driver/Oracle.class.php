...
class Oracle extends Db{
    ...
    public function __construct($config=''){
        ...
        //table表名前缀
        $this->tableprefix = $this->config['tblprefix'];
        //是否转换为小写
        $this->caselower = $this->config['caselower'];
    }

    /**
     * 连接数据库方法
     * @access public
     * @param $config 数据库config配置文件
     * @param $linkNum 连接池ID oracle默认1
     */
    public function connect($config='',$linkNum=1) {
     ...
     */
     public function execute($str) {
        ...
        if(preg_match("/^\s*(INSERT\s+INTO)\s+(\w+)\s+/i", $this->queryStr, $match)) {
            $this->table = C("DB_SEQUENCE_PREFIX") .str_ireplace($this->tableprefix, "", $match[2]);
    ...
    public function insertAll($datas,$options=array(),$replace=false) {
        ...
        foreach ($datas as $data){
            ...
            $sql   .=  ($replace?'REPLACE':'INSERT').' INTO '.$this->parseTable($options['table']).' ('.implode(',', $fields).') VALUES '.$value.';';
        }
