...
class Db {
    ...
    private function parseConfig($db_config='') {
        if ( !empty($db_config) && is_string($db_config)) {
            ...
            $db_config = array(
                ...
                'caselower' =>  $db_config['db_case_lower'],
            );
            ...
            if( C('DB_DSN') && 'pdo' != strtolower(C('DB_TYPE')) ) { // 如果设置了DB_DSN 则优先
                ...
                /*
                 * edit by wangbaoqing 2014-11-5
                 */
                $default_config = C('DB_CONFIG.DEFAULT_CONFIG');
                $db_config = array(
                    'dbms'      =>  $default_config['DB_TYPE'],
                    'username'  =>  $default_config['DB_USER'],
                    'password'  =>  $default_config['DB_PWD'],
                    'hostname'  =>  $default_config['DB_HOST'],
                    'hostport'  =>  $default_config['DB_PORT'],
                    'database'  =>  $default_config['DB_NAME'],
                    'tblprefix' =>  $default_config['DB_PREFIX'],
                    'dsn'       =>  $default_config['DB_DSN'],
                    'params'    =>  $default_config['DB_PARAMS'],
                    'charset'   =>  $default_config['DB_CHARSET'],
                    'caselower' =>  $default_config['DB_CASE_LOWER'],
                );
            }
        }
        return $db_config;
    ...
    protected function parseSet($data) {
        foreach ($data as $key=>$val){
            ...
            }elseif(is_array($val) && 'function' == $val[0]){
                $set[]  =   $this->parseKey($key).'='.$val[1];
    ...
    protected function parseValue($value) {
        ...
        }elseif(isset($value[0]) && is_string($value[0]) && strtolower($value[0]) == 'function'){
            $value =  $value[1];
    ...
    public function insert($data,$options=array(),$replace=false) {
        ...
        foreach ($data as $key=>$val){
            ...
            } else if (is_array($val) && $val[0]=='function') {
                //有数据库函数字段、值插入 --add by wangbaoqing 2014-11-21
                // -start-
                $fields[]   =  $this->parseKey($key);
                $values[]   =  $val[1];
                // -end-
