...
class View {
    ...
    public function parseTemplate($template='') {
        ...
        /** modify by wangbaoiqng 2014-7-16 start  **/
        // $file   =   THEME_PATH.$template.C('TMPL_TEMPLATE_SUFFIX');
        $file   =   THEME_PATH.$template;
        if(C('TMPL_LOAD_DEFAULTTHEME') && THEME_NAME != C('DEFAULT_THEME') && !is_file($file)){
            // 找不到当前主题模板的时候定位默认主题中的模板
            // $file   =   dirname(THEME_PATH).'/'.C('DEFAULT_THEME').'/'.$template.C('TMPL_TEMPLATE_SUFFIX');
            $file   =   dirname(THEME_PATH).'/'.C('DEFAULT_THEME').'/'.$template;
        }

        $tpl_suffix = C('TMPL_TEMPLATE_SUFFIX');
        $fileinfo = pathinfo($file);
        //如果模板文件的后缀名不等于默认的模板文件后缀名 为其添加后缀名
        if ('.'.$fileinfo['extension'] != $tpl_suffix) $file .= $tpl_suffix;
        /** modify by wangbaoiqng 2014-7-16 end **/
